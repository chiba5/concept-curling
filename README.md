...
- **ルール**（ロビーに表示される 4 項目）:
  ...
  2. AI が各概念とテーマの関連度を 0〜100 で採点します（**0 = 無関係、100 = 完全一致**）。合計が下限（既定 50）以上の概念だけをライフにできます（最大 3 つ、既定では全部が相手に見えない SECRET）
  3. 全員同時に攻撃概念を 1 つ出します。攻撃と相手ライフの関連度が**閾値（既定 50）を超えたら**そのライフは破壊されます（遠すぎる攻撃は届きません）
