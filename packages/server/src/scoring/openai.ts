...
- 低すぎる方向に偏らないこと。連想が一言で説明できるなら必ず 50 以上を付ける。
- アンカー例（この水準に較正すること）:
  100 = 同一語（虹と虹）
  90〜99 = ほぼ同義・専用の強い結び付き（野球とホームラン 95 / 魔法とハリーポッター 98）
  70〜89 = 明確な連想・部分/全体・形や用途の強い共通（虹とアーチ 75 / 森林と年輪 80 / 魔法とドラクエ 90）
  50〜69 = はっきり説明できる連想・同カテゴリの別物（野球と応援歌 60 / 虹と雨上がり 65 / 掃除機と洗濯機 55）
  30〜49 = 遠いが説明可能な連想（ピアノと虹 35 — どちらも「色彩・階調」の比喩でつながる）
  0〜29 = ほぼ無関係（野球とピアノ 10）
- 同カテゴリ・同じ場面で使う・部分と全体、のどれかが成り立つペアを 30 未満にしないこと。
...
目的: 各テーマと「関連はあるが推測されにくい」概念（各テーマと関連度 25〜45 目安。合計が選抜下限を
わずかに超える“遠さ”が理想 — 近いほど相手に当てられる。どれかのテーマと 60 以上になる直接的な語は不可）を出す。
重要: どれか 1 つのテーマだけに寄せない。あるテーマと無関係（20 未満）な語は合計が選抜下限を割って
ライフにできない。**全テーマそれぞれに説明可能な連想の橋を持つ語**を選ぶ（例: テーマ「ギター×掃除機」
なら「コンセント」= ギターはアンプの電源・掃除機も電源、のように両方へ繋がる語）。
厳守:
